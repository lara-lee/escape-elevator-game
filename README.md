...
- 🚪 **문 닫기**(버튼 / `Space` · `↓` · `Enter`)를 눌러 **3초** 버티면 → 한 층 **아래로** 이동.
- 닫는 도중 승객이 도착하면 **실패** → 문이 다시 열리고 **하트 −1**.
- 문이 열려 있을 땐 승객이 그냥 타고 사라짐 → **빈 틈을 노려** 누르기.
- ❤️ **하트**를 모두 잃으면 **게임 오버**(야근). 시작 하트 3개(4·5단계는 4·5개).
- ❤️ **보너스 하트가 하늘에서 떨어지면** **탭(터치)해서 잡기** — 단계마다 단계 번호만큼 등장(못 잡으면 못 받음).
- 🏁 **1층 로비 도착 = 퇴근 성공**(클리어).
- 게임 시작 시 **3·2·1·START** 카운트다운. 일시정지: ⏸ 버튼 / `Esc` · `P`.
